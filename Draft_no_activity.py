...
# COOKING ACTIVITY REMOVED FROM OBSERVATION STATE
class SmartVentilationEnv(gym.Env):
    ...
    def __init__(self, alpha=1.0, beta=1.0, gamma=1.0, delta=1.0):
        ...
        # 1. Norm_VOC, 2. Norm_PM, 3. Norm_CO2, 4. Norm_AirFlow
        obs_low = np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32)
        obs_high = np.array([np.inf, np.inf, np.inf, 1.0], dtype=np.float32)
    ...
    def _get_obs(self):
        ...
        return np.array([n_voc, n_pm, n_co2, n_flow], dtype=np.float32)
